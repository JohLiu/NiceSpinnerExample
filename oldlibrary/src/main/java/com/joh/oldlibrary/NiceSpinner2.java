 ...
 * 自定义Spinner
 * <p>
 * 1.该Spinner可传递两种数据，对应两种Adapter
 * (1)NiceSpinnerAdapterWrapper,使用这种adapter用于简单的单数据类型
 * (2)NiceSpinnerAdapter<T>，该adapter用于自定义的未知实体类类型，使用时传建一个类似的adapter，
 * 将对象更改为对应的实体类，并将getItem(position)的返回数据该为需要显示的数据即可
 * </>
 *
 * @author : Joh Liu
 * @date : 2019/7/26 13:11
 */
public class NiceSpinner2 extends AppCompatTextView {
    ...
    private int dropDownShowCount = 6; //默认显示几行下拉
    private
    @DrawableRes
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        backgroundSelector = typedArray.getResourceId(R.styleable.NiceSpinner_backgroundSelector, R.drawable.spinner_nomal_bg);
        backgroundPop = typedArray.getResourceId(R.styleable.NiceSpinner_backgroundSelector, R.drawable.spinner_sel_bg);
...
//        listView.setDividerHeight(1);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            popupWindow.setBackgroundDrawable(ContextCompat.getDrawable(context, R.drawable.spinner_nomal_popup_bg));
        } else {
            popupWindow.setBackgroundDrawable(ContextCompat.getDrawable(context, R.drawable.spinner_nomal_popup_bg));
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (adapter != null && adapter.getCount() > 0) {
            if (adapter.getCount() < dropDownShowCount) {
                ...
                popupWindow.setHeight(listView.getMeasuredHeight() * adapter.getCount());
            } else {
                int h = getHeight();
                int w = getWidth();
                listView.measure(w, h);
                popupWindow.setWidth(w);
                popupWindow.setHeight(listView.getMeasuredHeight() * dropDownShowCount + listView.getDividerHeight() * (dropDownShowCount - 1));
            }
        }
     ...
     * @return
     */
    public int getDropDownShowCount() {
        return dropDownShowCount;
    }

    public void setDropDownShowCount(int dropDownShowCount) {
        this.dropDownShowCount = dropDownShowCount;
    }
     ...
     * @param txtHeader
     */
    public void setNoSel(String txtHeader) {
        // If the adapter needs to be settled again, ensure to reset the selected index as well
        selectedIndex = -1;
        setText(txtHeader);
    }
